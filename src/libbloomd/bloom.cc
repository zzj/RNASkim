...
void bf_internal_compute_hashes(uint32_t k_num, const char *key, uint64_t len, uint64_t *hashes);
...
int bf_add(bloom_bloomfilter *filter, char* key) {
    ...
    return bf_add(filter, key, len);
}

int bf_add(bloom_bloomfilter *filter, const char* key, uint64_t len) {
    // Allocate the hash space
    uint64_t *hashes = alloca(filter->header->k_num * sizeof(uint64_t));

    // Compute the hashes
    bf_internal_compute_hashes(filter->header->k_num, key, len, hashes);
...
int bf_contains(bloom_bloomfilter *filter, char* key) {
    uint64_t len = strlen(key);
    return bf_contains(filter, key, len);
}

int bf_contains(bloom_bloomfilter *filter, const char* key, uint64_t len) {
    // Allocate the hash space
    uint64_t *hashes = alloca(filter->header->k_num * sizeof(uint64_t));
...
void bf_compute_hashes(uint32_t k_num, char *key, uint64_t *hashes) {
    ...
    uint64_t len = strlen(key);
    bf_internal_compute_hashes(k_num, key, len, hashes);
}
